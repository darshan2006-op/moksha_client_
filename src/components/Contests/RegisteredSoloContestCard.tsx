...
import type { ClubSlug, Image } from '~/types'
...
const RegisteredSoloContestCard = memo(({ clubSlug, contestSlug }: RegisteredSoloContestCardProps) => {
  ...
  // If contest is null or undefined, don't render anything
  if (isNullOrUndefined(contest)) {
    console.error(`Contest not found for club: ${clubSlug}, contest: ${contestSlug}`)
    return null
  }
    ...
    <MLink to={link} className='block'>
      <Sheet className='flex flex-row-reverse lg:flex-row overflow-hidden'>
        <div className='h-36 w-36 relative'>
          ...
          {!isNullOrUndefined(contest.image) ? (
            <Picture picture={contest.image} alt={`moksha-contest-${contest.slug}-poster`} />
          ) : (
            <div className="w-full h-full bg-amber-900/50 flex items-center justify-center text-amber-500">
              No Image
            </div>
          )}
        </div>
